...
Hosts can be saved to and loaded from the cloud. MAMP PRO will save both your document root folder and database data. Your data for each individual host will be saved in a single zip file in `~/Dropbox/Apps/MAMP PRO`. Confirm which databases are linked to your host on the [Databases tab](). 
